...
qemu-doc - QEMU version 2.12.91 User Documentation
...
=item B<s390-squash-mcss=on|off>
...
Enable virtio balloon device, optionally with PCI address I<addr>. This
option is deprecated, use B<-device virtio-balloon> instead.
...
parameter is deprecated, use the corresponding parameter of C<-device>
instead.

=item B<addr=>I<addr>

Specify the controller's PCI address (if=virtio only). This parameter is
deprecated, use the corresponding parameter of C<-device> instead.
...
=item B<-object memory-backend-memfd,id=>I<id>B<,merge=>I<on|off>B<,dump=>I<on|off>B<,prealloc=>I<on|off>B<,size=>I<size>B<,host-nodes=>I<host-nodes>B<,policy=>I<default|preferred|bind|interleave>B<,seal=>I<on|off>B<,hugetlb=>I<on|off>B<,hugetlbsize=>I<size>
